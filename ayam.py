...
# PERBAIKAN PENTING: Menggunakan __file__ (double underscore kiri kanan)
# Ini memperbaiki error "Unknown" karena model sekarang bisa ditemukan
...
def load_classifier_model():
    try:
        ...
        st.error("❌ Gagal load model. Pastikan file .keras ada di folder yang sama.")
        print(e)
        return None

model = load_classifier_model()

# Jika model gagal load, hentikan aplikasi dan beri pesan error
if model is None:
...
def ask_gemini(messages):
    ...
    # System instruction dikirim via config
    ...
    for api_key_name, api_key in api_keys_to_try:
        try:
            ...
            if api_key_name != api_keys_to_try[0][0]:
                st.success(f"✅ Menggunakan {api_key_name} (Backup)")
            ...
            if "quota" in error_msg or "limit" in error_msg or "429" in error_msg:
                if api_key != api_keys_to_try[-1][1]:
                    ...
                    return "Maaf, semua quota API telah habis."
            else:
                return f"Maaf, terjadi kesalahan: {str(e)}"
...
def render_detection_page():
    ...
    # --- POSISI UPLOAD DIPINDAHKAN KE ATAS ---
            ...
            if not valid_predictions:
                ...
                for idx, res in enumerate(results):
                    with cols[idx % 3]:
                        ...
                        st.caption(f"Score: {res['final_score']:.2f}")
    ...
    # --- TOMBOL NAVIGASI DILETAKKAN PALING BAWAH ---
    st.divider()
...
def render_chat_page():
    ...
    if not GEMINI_API_KEY_1 and not GEMINI_API_KEY_2:
        st.warning("⚠️ Fitur Chatbot belum aktif. Masukkan GEMINI_API_KEY_1 di Secrets.")
    else:
        if "messages" not in st.session_state or len(st.session_state.messages) == 0:
            if "prediction_context" in st.session_state and st.session_state.prediction_context:
                ...
                st.session_state.messages = [
                    {"role": "assistant", "content": "Halo! Silakan upload gambar di halaman Deteksi terlebih dahulu."}
                ]
